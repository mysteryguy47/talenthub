...
export default function BackendTest() {
  ...
  const testBackend = async () => {
    ...
    // Test 3: Direct backend URL
    try {
      addResult("Test 3: Testing direct http://localhost:8000/api/health...", 'info');
      const directRes = await fetch("http://localhost:8000/api/health");
      const directText = await directRes.text();
      if (directRes.ok) {
        addResult(`✅ Direct backend accessible: ${directText}`, 'success');
      } else {
        addResult(`❌ Direct backend failed: ${directRes.status}`, 'error');
      }
    } catch (e: any) {
      addResult(`❌ Direct backend error: ${e.message}`, 'error');
    }
