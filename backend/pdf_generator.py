...
def render_vertical_question_compact(canvas, question, x: float, y: float, width: float, question_num: int, with_answers: bool = False) -> float:
    ...
    # Check if this is decimal_add_sub (operator is "±" and operands are stored as integers * 10)
    # More robust detection: check operator string and Unicode code point
    operator_str = str(question.operator) if hasattr(question, 'operator') and question.operator else ""
    is_decimal_add_sub = (
        operator_str == "±" or 
        operator_str == "\u00B1" or
    ...
    # ALWAYS convert operands for decimal_add_sub (don't rely on text field)
    # This ensures decimals are always shown
